...
<script setup lang="tsx">
  ...
  import type {
    TransType
  } from '@/hooks/auto-imports/useTrans.ts'
  import type {
    UseDialogExpose
  } from '@/hooks/useDialog.ts'

  import {
    deleteByIds,
    page
  } from '~/{{ $packageName }}/api/{{ $camelCaseName }}.ts'
  ...
  import {
    ResultCode
  } from '@/utils/ResultCode.ts'

  import Form from './form.vue'

  defineOptions({
    name: '{{$packageName}}:{{Str::snake($camelCaseName)}}'
  })
  ...
  const maDialog: UseDialogExpose = useDialog({
    ...
    ok: ({
      formType
    }, okLoadingState: (state: boolean) => void) => {
      okLoadingState(true)
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['{{$packageName}}:{{Str::snake($camelCaseName)}}:create']"
          type="primary"
          @click="() => {
      ...
      <template #empty>
        <el-empty>
          <el-button
            v-auth="['{{$packageName}}:{{Str::snake($componentName)}}:create']"
            type="primary"
            @click="() => {
              maDialog.setTitle(t('crud.add'))
              maDialog.open({ formType: 'add' })
