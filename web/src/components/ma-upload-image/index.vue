...
<template>
  <el-upload v-model:file-list="fileList" :class="`ma-upload-${id}`" :before-upload="beforeUpload"
    :http-request="uploadLocal" :on-success="handleSuccess" :on-exceed="handleExceed" :on-error="handleError"
    :multiple="multiple" :limit="limit" v-bind="$attrs">
    <slot name="default">
    ...
    <template #file="{ file, index }">
      <div class="ma-preview-list ma-upload-container relative" :style="getSize">
        <div class="ma-preview-mask">
          <ma-svg-icon name="weui:eyes-on-filled" class="icon" :size="20"
            @click="() => ElImageRefs?.$el?.children[0]?.click?.()" />
          <ma-svg-icon name="material-symbols:delete" class="icon" :size="20" @click="() => {
            fileList.splice(index, 1)
            updateModelValue()
          }" />
        </div>
        <el-image ref="ElImageRefs" :src="file?.url" class="absolute rounded-md" :style="getSize" fit="cover"
          :zoom-rate="1.2" :max-scale="7" :min-scale="0.2" :preview-src-list="previewList" :initial-index="index" />
      </div>
      <component :is="btnRender()" v-if="index === (fileList.length - 1) && multiple && fileList.length < limit"
        class="cursor-pointer" @click="() => uploadBtnRef?.click?.()" />
    </template>
    <template #tip>
    ...
    <MaResourcePicker v-model:visible="isOpenResource" :multiple="multiple" :limit="limit" @confirm="(selected) => {
...
.ma-upload-container {
  @apply flex items-center justify-center bg-gray-50 b-1 b-dashed rounded-md b-gray-3 dark-b-dark-50 transition-all duration-300 text-gray-5 dark-bg-dark-5 relative;
  ;

  .ma-resource-btn {
    @apply absolute top-0 b-1 b-dashed b-gray-3 dark-b-dark-50 transition-all duration-300 rounded-t-md w-[calc(100%)] mx-auto b-t-0 b-l-0 b-r-0 text-gray-5 dark-bg-dark-8 bg-gray-1 h-[calc(100%-80%)] flex items-center justify-center;
  }
