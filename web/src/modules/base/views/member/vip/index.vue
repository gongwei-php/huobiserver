...
import MemberVipForm from './form.vue'
import { deleteByIds, page } from '~/base/api/membervip'
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('baseMemberVipManage.mainTitle'),
    subTitle: () => t('baseMemberVipManage.subTitle'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button v-auth="['member:vip:save']" type="primary" @click="() => {
      ...
      <template #toolbarLeft>
        <el-button v-auth="['member:vip:delete']" type="danger" plain @click="handleDelete">
          {{ t('crud.delete') }}
        </el-button>
      </template>
    </MaProTable>
    ...
    <component :is="maDialog.Dialog">
      <template #default="{ formType, data }">
        <!-- 新增、编辑表单 -->
        <MemberVipForm v-if="['add', 'edit'].includes(formType)" ref="formRef" :form-type="formType" :data="data" />
      </template>
    </component>
  </div>
</template>
