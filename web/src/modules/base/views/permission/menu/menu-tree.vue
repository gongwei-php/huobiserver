...
<template>
  <ma-tree v-bind="$attrs" ref="maTreeRef" tree-key="meta.title" highlight-current :expand-on-click-node="false"
    node-key="id" :indent="26" auto-expand-parent class="mt-1 h-[200px] lg:h-[calc(100%-80px)]" @node-click="(node: MenuVo) => {
    ...
    <template #default="{ node, data }">
      <div class="mine-tree-node">
        ...
        <div class="do" :class="{ '!inline-block': maTreeRef.elTree.getCurrentKey() === data.id }">
          ...
          <el-button v-if="data.meta?.type === 'M'" v-auth="['permission:menu:save']" size="small" circle type="primary"
            @click.stop="addMenu(data)">
            <ma-svg-icon name="ic:round-plus" :size="20" />
          </el-button>
          <el-popconfirm :title="t('crud.delDataMessage')" :confirm-button-text="t('crud.ok')"
            :cancel-button-text="t('crud.cancel')" @confirm.stop="async () => {
            ...
            <template #reference>
              <el-button v-auth="['permission:menu:delete']" size="small" circle type="danger">
    ...
    <template #extra>
      <el-button type="primary" class="w-full" @click="() => emit('menu-select', newMenu)">
